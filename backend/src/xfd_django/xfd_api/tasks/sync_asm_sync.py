        ...
        for data_source in data_sources:
            DataSource.objects.get_or_create(
                ...
                defaults={
                    ...
                    'last_run': datetime.datetime.fromisoformat(data_source.get('last_run')).date()
                }
            )
...
def flag_asset_changes(org):
    ...
    IpsSubs.objects.filter(last_seen__lt=cutoff_date, ip__organization=org).update(current=False)

    Ip.objects.filter(last_seen_timestamp__lt=cutoff_date, organization=org).update(current=False)
