...
export const VSDashboardGate: React.FC<{
  contextType: { filters: any; removeFilter: any };
  children: React.ReactNode;
}> = (props) => {
  const { filters, removeFilter } = props.contextType;
  const { currentOrganization, user } = useAuthContext();

  useClearFiltersOnMount(filters, removeFilter);

  const { orgId } = useOrgInfo(filters, currentOrganization);
  ...
  return (
    <VulnerabilityScanWithSearch orgId={orgId} vulnScanData={vulnScanData} />
  );
};
