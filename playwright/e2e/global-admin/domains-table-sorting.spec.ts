...
test.describe('domains-table', () => {
  test('IP column sorts with server-side sorting', async ({ page, makeAxeBuilder }, testInfo: TestInfo) => {
    await page.goto('/inventory/domains');
    await page.waitForSelector('[aria-label="Domains Table"]');
    
    // Click IP column header to sort
    await page.getByRole('columnheader', { name: /IP/i }).click();
    
    // Wait for the table to update after server-side sort
    await page.waitForLoadState('networkidle');
    
    // Get sorted IP values using the correct selector
    const sortedIpCells = await page.getByRole('gridcell', { name: /IP Address for Domain/ }).allTextContents();
  ...
  test('Domain column sorts with server-side sorting', async ({ page, makeAxeBuilder }, testInfo: TestInfo) => {
    await page.goto('/inventory/domains');
    await page.waitForSelector('[aria-label="Domains Table"]');
    
    // Click Domain column header to sort
    ...
    const sortedDomainCells = await page.getByRole('gridcell', { name: /Domain Name:/ }).allTextContents();
